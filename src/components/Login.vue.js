    ...
    methods: {
        login() {
            ...
            setTimeout(() => {
                ...
                let user = this.fakeUsers.find((x) => x.username == this.user.username && x.password === this.user.password);
                if(user) {
              ...
              <v-card class="elevation-12">
                    ...
                    <v-card-text>
                        <v-form @keyup.enter.native="login">
                            ...
                            <v-btn :color="themeColor + ' white--text'" :loading="loading" block="block" @click.prevent="login" type="button" :disabled="loading">Sign in</v-btn>
                        </v-form>
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
